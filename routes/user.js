...
router.post('/signup', async (req,res)=>{
   try{
    ...
    const password = await bcrypt.hash(req.body.password, 10); /// pass ko hash krne k liye method ///
     ...
     const user=  new User({...req.body,password})  //password ander ae ga
 ...
 router.post('/userlogin', async (req,res)=>{
  ...
  const token= jwt.sign({_id:user._id, email:user.email},process.env.JWT_SECRET)//jo save krwnana wo sign mein dena hai aur next secret key///
